...
STYLE_TAGS = [
    # Most useful / popular tags first
    "(none)",
    "<whisper>", "<sleepy>", "<laughing>", "<laugh>", "<fast>",
    "<sad>", "<angry>", "<happy>", "<sarcastic>", "<confused>",
    # Other emotions
    "<awe>", "<bored>", "<calm>", "<desire>", "<disgusted>",
    "<fearful>", "<sympathetic>",
    # Delivery / paralinguistic
    "<breath>", "<narration>", "<articulated>", "<enunciated>",
    "<projected>", "<reduced>",
]
...
def synthesize(text, style_tag, voice_label, uploaded_audio,
               temperature, top_k, top_p, eos_bias, max_frames, min_frames):
    if not text or not text.strip():
        return None, "⚠️ Please enter some text."

    # Prepend style tag (if any) so the model conditions on it during generation.
    if style_tag and style_tag != "(none)":
        text = f"{style_tag} {text.strip()}"
...
Expressive multilingual voice-cloning TTS with **23 style tags**
(e.g. `<happy>`, `<sad>`, `<whisper>`, `<sarcastic>`).
[Qwen2.5-0.5B](https://huggingface.co/Qwen/Qwen2.5-0.5B) backbone +
[Kyutai Mimi](https://huggingface.co/kyutai/mimi) neural codec, fine-tuned on
[Expresso](https://huggingface.co/datasets/ylacombe/expresso).

**1.** Pick a style tag · **2.** Enter text · **3.** Pick a voice reference
(any speaker, any language) · **4.** Generate

Running on **{DEVICE.upper()}**. On the free CPU-basic tier, expect **~30–90 s per short utterance**.

> Trained on limited expressive data — quality is **decent, not perfect**.
> Style tags were only seen alongside English text in fine-tuning.

- [Model](https://huggingface.co/shangeth/Wren-TTS-0.5B-multi-expressive) · [Code](https://github.com/shangeth/wren-tts)
"""

with gr.Blocks(title="Wren-TTS-0.5B-multi-expressive", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        with gr.Column(scale=3):
            ...
            style_in = gr.Dropdown(
                ...
                choices=STYLE_TAGS,
                value="(none)",
                info="Prepended to the text. Trained on Expresso (English-only); cross-lingual behaviour is undefined.",
            )
            voice = gr.Radio(
    ...
    go.click(
        synthesize,
        inputs=[text_in, style_in, voice, upload, temperature, top_k, top_p, eos_bias, max_frames, min_frames],
    ...
    if VOICE_CHOICES and VOICE_CHOICES[0] != "Upload my own":
        ...
        gr.Examples(
            label="Click an example to pre-fill (style + multilingual)",
            examples=[
                # Expressive English
                ["Welcome to the show, everybody!",                                "<happy>",     v0],
                ["This is the saddest day of my life.",                            "<sad>",       v1],
                ["I will tell you a secret. Don't tell anyone.",                   "<whisper>",   v2],
                ["Oh sure, that worked out perfectly.",                            "<sarcastic>", v0],
                ["I cannot believe what just happened.",                           "<confused>",  v1],
                ["Once upon a time, in a land far away.",                          "<narration>", v2],
                ["I'm so tired, I can barely keep my eyes open.",                  "<sleepy>",    v0],
                # Multilingual (no tag — base voice cloning)
                ["Hallo Welt, wie geht es dir heute?",                             "(none)",      v0],
                ["Bonjour le monde, comment ça va aujourd'hui ?",                  "(none)",      v1],
                ["Hola mundo, ¿cómo estás hoy?",                                   "(none)",      v2],
                ["Ciao mondo, come stai oggi?",                                    "(none)",      v0],
            ],
            inputs=[text_in, style_in, voice],
        )
